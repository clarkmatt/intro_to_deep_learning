...
def wsj_collate(batch):
    ...
    has_transcript = batch[0][1] is not None
    ...
    if has_transcript:
        ...
        return utterance_tensor, utterance_lengths
...
class Speller(nn.Module):
    ...
    def forward(self, listener_features, utterance_lengths, transcript=None, teacher_force=1.0):
        ...
        for char_idx in range(max_transcript_length):
...
def random_search():
    ...
    batch_size = 1
    if torch.cuda.is_available():
        test_loader = torch.utils.data.DataLoader(test_set, batch_size=batch_size, shuffle=False, collate_fn=wsj_collate, pin_memory=True)
    else:
    ...
    model_file = "las.pt"
    las_model.load_state_dict(torch.load(model_file).state_dict())
    if torch.cuda.is_available():
        las_model.cuda()
    las_model.eval()
    ...
    for idx, (utterance_tensor, utterance_lengths) in enumerate(test_loader):
        ...
        for i in range(500):
            ...
            transcript_tensor = index.unsqueeze(1)
            transcript_lengths = np.asarray([len(index)])

            # Get loss
            char_logits, _, _ = las_model(packed_utterance, transcript_tensor)
            char_logits = torch.cat(char_logits, 1)
            ...
            if loss.data[0] < best_loss:
                ...
                best_transcript = "".join(vocab[index.data[:-1]])
        
        #print("".join(vocab[transcript_tensor.data[:,0]]))
        generated_transcripts.append(best_transcript)
        print(idx)
...
if __name__=="__main__":
    transcripts = random_search()
    with open('my_submission.csv', 'w') as f:
        f.write("Id,Predicted\n")
        for idx, transcript in enumerate(transcripts):
            f.write("{},{}\n".format(idx, transcript))
